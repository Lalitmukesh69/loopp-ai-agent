...
const MarqueeRow = ({ items, direction, duration }) => {
  // We use dynamic animation classes based on the direction. 
  // No pause on hover is added here, so it continues scrolling seamlessly.
  const animationClass = direction === 'left' 
    ? `animate-[marqueeLeft_${duration}s_linear_infinite]` 
    : `animate-[marqueeRight_${duration}s_linear_infinite]`;
...
export default function GetAnythingDoneApp() {
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F4] font-sans selection:bg-[#BAC66E] selection:text-black pt-24 pb-24 flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="relative w-full max-w-[1400px] mx-auto overflow-hidden">
        ...
        <div className="absolute inset-y-0 left-0 w-24 md:w-48 bg-gradient-to-r from-[#F9F8F4] via-[#F9F8F4]/90 to-transparent z-10 pointer-events-none"></div>
        <div className="absolute inset-y-0 right-0 w-24 md:w-48 bg-gradient-to-l from-[#F9F8F4] via-[#F9F8F4]/90 to-transparent z-10 pointer-events-none"></div>
        ...
        <div className="flex flex-col gap-2 w-full relative -ml-12 md:-ml-24">
          ...
          <div className="w-full">
            <MarqueeRow items={row1Items} direction="right" duration="45" />
          </div>

          {/* Row 2 - Right to Left */}
          <div className="w-full">
            {/* Slightly faster duration to create a nice parallax variation */}
            <MarqueeRow items={row2Items} direction="left" duration="40" />
          </div>

          {/* Row 3 - Left to Right */}
          <div className="w-full -ml-8">
            <MarqueeRow items={row3Items} direction="right" duration="50" />
          </div>
